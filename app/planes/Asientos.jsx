      ...
      try {
        ...
        if (asientos && asientos.length > 0) {
          ...
          asientos.forEach((asiento) => {
            ...
            if (asiento && asiento.Fila && asiento.Columna) {
              ...
              console.log(
                `Asiento ${seatId}: estado = ${asiento.estado}, disponible = ${estados[seatId]}`,
              );
            } else {
        ...
        filas.forEach((fila) => {
          columnas.forEach((col) => {
            const seatId = `${fila}${col}`;
            console.log(
              `Estado final de asiento ${seatId}: ${estados[seatId]}`,
            );
          });
        });
          ...
          {["A", "B", "C"].map((row) => (
            <View key={row} className="flex-row mb-4">
              {[1, 2, 3, 4, 5].map((col) => {
                ...
                console.log(
                  `Renderizando asiento ${seatId}: seleccionado = ${seleccionado}, disponible = ${disponible}`,
                );
